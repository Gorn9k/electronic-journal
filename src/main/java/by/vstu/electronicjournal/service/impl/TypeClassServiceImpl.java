...
public class TypeClassServiceImpl
    ...
    @Override
    public List<TypeClass> search(String query) {
        if (query.isEmpty()) {
            return typeClassRepository.findAll();
        }
        return typeClassRepository.findAll(getSpecifications(query));
    }

    @Override
    public List<TypeClassDTO> searchAndMapInDTO(String query) {
        if (query.isEmpty()) {
            return findAll();
        }
